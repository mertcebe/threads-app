...
const SingleCommentContainer = ({ comment, type }) => {
    ...
    const deleteComment = () => {
        if (!searchParams2) {
            ...
            deleteCommentInOf(comment.postId, startCommentInComment.commentIn.ownerUid, startCommentInComment?.commentIn.id, comment.id)
                .then(() => {
                    setIsDeleted(true);
                    refreshAllComments(dispatch, !isRefreshComments);
                })
    ...
    return (
        <div style={{ display: "flex", alignItems: "start", margin: type === 'reply' ? '10px 0' : '0', padding: "10px 14px", borderRadius: "10px", position: "relative", opacity: isDeleted ? '0.4' : '1', pointerEvents: isDeleted && 'none' }}>
            <div>
            ...
            <div style={{ marginLeft: "10px" }}>
                ...
                </div>
                {
                    type !== 'reply' ?
                        ...
                        :
                        <button className='text-secondary' style={{ opacity: "0.8", fontSize: "12px", border: "none", outline: "none", background: "transparent" }} onClick={() => {
                            if (comment.type === 'comment') {
                                navigate(`/post?id=${comment.postId}`)
                            }
                            else if (comment.type === 'commentIn') {
                                navigate(`/post?id=${comment.postId}&commentTo=${comment.commentTo.commentId}`)
                            }
                        }}>
                            Go to post <i className="fa-solid fa-arrow-right"></i>
                        </button>
                }
                {
                    type !== 'commentIn' ?
